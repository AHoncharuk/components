...
gulp.task('serve', ['build'], () => {
  ...
  gulp.watch('/fonts/*.{ttf,woff,woff2,eot,svg}', {
    cwd: dirs.srcPath
  }, ['watch:fonts'])
  ...
  const spriteSvgPath = `${dirs.srcPath}${dirs.blocksDirName}/sprite-svg/svg/`
  if (projectConfig.blocks['sprite-svg'] !== undefined) {
    gulp.watch('*.svg', {
      cwd: spriteSvgPath
    }, ['watch:sprite:svg']) // следит за новыми и удаляемыми файлами
  }
  ...
  if (projectConfig.blocks['sprite-png'] !== undefined) {
    gulp.watch('*.png', {
      cwd: spritePngPath
    }, ['watch:sprite:png']) // следит за новыми и удаляемыми файлами
  }
